<template>
  <nav class="bg-blueGray-500">
    <div class="container flex items-center justify-between mx-auto">
      <div class="flex items-center space-x-4">
        <noto:videocassette class="w-16 h-16" />
        <p
          class="text-4xl tracking-tight text-transparent bg-gradient-to-r from-blue-300 via-pink-400 to-red-400 font-bold bg-clip-text"
        >
          movie<span class="font-bold">SPOT</span>
        </p>
      </div>
      <div>
        <div v-if="isAuthenticated" class="flex items-center space-x-4">
          <p class="text-coolGray-200">Welcome {{ user?.displayName }}</p>
          <mdi:account class="text-white"/>
          <button
            @click="out"
